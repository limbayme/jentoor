...
function BrandLabel() {
  const texture = useMemo(() => {
    ...
    ctx.fillStyle = '#f5efdf'; ctx.fillRect(0, 0, 768, 1024);
    ctx.strokeStyle = '#cebda0'; ctx.lineWidth = 2;
    ctx.textAlign = 'center'; ctx.fillStyle = '#765337';
    ctx.font = 'italic 600 92px Georgia'; ctx.fillText('jentoor', 384, 240);
    ctx.fillStyle = '#c39a66'; ctx.fillRect(344, 328, 80, 5);
    ctx.font = '28px Arial'; ctx.fillText('LIQUID FORMULA', 384, 440);
    ctx.font = '64px Georgia'; ctx.fillStyle = '#765337'; ctx.fillText('Daily Ritual', 384, 554);
    ctx.font = '24px Arial'; ctx.fillStyle = '#9a8063'; ctx.fillText('FORMULATION STUDY', 384, 653);
    ctx.beginPath(); ctx.moveTo(128, 790); ctx.lineTo(640, 790); ctx.stroke();
    ctx.font = '26px Arial'; ctx.fillStyle = '#765337'; ctx.fillText('THE NUTRITION ATELIER', 384, 875);
...
export default function LiquidProductScene({expanded}:{accent:string;expanded:boolean}) {
  ...
  return <group>
    <group position={[-.36,-1.48,-.15]}>
      <mesh><latheGeometry args={[body,96]}/><meshPhysicalMaterial color="#b47b3c" roughness={.28} metalness={0} clearcoat={.65} clearcoatRoughness={.25}/></mesh>
      <BrandLabel/>
      <Ring radius={.286} y={2.46} tube={.017} color="#b6a16f"/>
      <group position={[0,2.49+(expanded?.1:0),0]}>
        <mesh><latheGeometry args={[cap,80]}/><meshPhysicalMaterial color="#dbccaf" roughness={.65} metalness={0} clearcoat={.1}/></mesh>
        <Ring radius={.332} y={.047} tube={.008} color="#bea879"/>
        <Cylinder radius={.24} height={.005} position={[0,.347,0]} color="#e5d8bf" metalness={0}/>
      </group>
    </group>
    <group position={[.93,-1.45,.54]} rotation={[0,.1,0]}>
      <mesh><latheGeometry args={[cup,64]}/><meshPhysicalMaterial color="#f3e9d4" transparent opacity={.31} roughness={.12} metalness={.03} clearcoat={1} depthWrite={false}/></mesh>
      <Cylinder radius={.278} height={.24} position={[0,.22,0]} color="#a97531" metalness={.02}/>
      <Ring radius={.322} y={.643} tube={.015} color="#d7cbb5"/>
